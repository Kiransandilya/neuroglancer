...
print("*" * 40)
from os import path
finalpath = path.join("data\Folder\Folder", images[0])
...
finalpath2 = path.join("data\Folder\Folder", images[1])
print("*"*40)
with open(finalpath2) as f:
    lines = f.readlines()
    la = [i for i in lines if "tif" in i.lower()]
    z_axis = la[0].split()
...
slices = len(counter)
image = cv2.imread(os.path.join(temp_folder_path_0,counter[0]))
